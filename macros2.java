...
//For Average of temperature of Vall and RK3
package macro;
...
public class macros2 extends StarMacro {
  ...
  private void execute0() {
    ...
    /*for (int g = 1; g <= 4; g++) {
        ...
        Region region_0 = 
          simulation_0.getRegionManager().getRegion("Vall_CT_RK-III");
       ...
       NullFieldFunction nullFieldFunction_0 = 
        ...
        thresholdPart_4.getInputParts().setObjects(region_0);
        thresholdPart_4.setFieldFunction(vectorComponentFieldFunction_Z);
        thresholdPart_4.getRangeQuantities().setArray(new DoubleVector(new double[] {Lmin, Lmax}));
       ...
       /* AreaAverageReport areaAverageReport_1 = 
        ...
        userFieldFunction_Sum.setPresentationName("ModifTem_sum");
        userFieldFunction_Sum.setFunctionName("ModifTem_sum");
        ...
        for (int i=1; i<=nx;i++){    
            ...
            RightBoundariesR = maxReport_1.getValue();


            for (int j = 1; j <= nr; j++) {
                StringBuilder FieldFunctionBuilder = new StringBuilder();
                LeftBoundarisR = RightBoundariesR - deltaR;
                ...
                if (massiveReports[j].getValue() != 0) {
                    ...
                    massiveOfFieldFunction[j].setPresentationName("ModifTem");
                    massiveOfFieldFunction[j].setFunctionName("ModifTem");
                    ...
                    if (j == 1) {
                    FieldFunctionBuilder.append(String.format("(($${Position}(\"Cylindrical 1\")[0]<=%f) && ($${Position}(\"Cylindrical 1\")[0]>=%f) && ($${Position}(\"Cylindrical 1\")[2]<%f) &&($${Position}(\"Cylindrical 1\")[2]>%f))?%f:0", RightBoundariesR+0.001, LeftBoundarisR,RightBoundariesZ,LeftBoundarisZ, massiveReports[j].getValue()));
                    } 
                    if (j == nr) {
                    FieldFunctionBuilder.append(String.format("(($${Position}(\"Cylindrical 1\")[0]<=%f) && ($${Position}(\"Cylindrical 1\")[0]>=%f) && ($${Position}(\"Cylindrical 1\")[2]<%f) &&($${Position}(\"Cylindrical 1\")[2]>%f))?%f:0", RightBoundariesR, LeftBoundarisR-0.001,RightBoundariesZ,LeftBoundarisZ, massiveReports[j].getValue()));
                    }
                    if ( j != 1 && j != nr) {
        ...
        userFieldFunction_SumAndTem.setPresentationName("ModifTem_itog");
        userFieldFunction_SumAndTem.setFunctionName("ModifTem_itog");
        userFieldFunction_SumAndTem.setDimensions(Dimensions.Builder().temperature(1).build());
        userFieldFunction_SumAndTem.setDefinition(String.format("($${Position}(\"Cylindrical 1\")[2]> %f && $${Position}(\"Cylindrical 1\")[2]<%f)? (${ModifTem_sum}):${Temperature}", Lmin, Lmax));
        /*if (flagTemOrHTC) {
            userFieldFunction_SumAndTem.setDimensions(Dimensions.Builder().temperature(1).build());
      ...
      */
        ...
        */
        simulation_0.getPartManager().removeObjects(thresholdPart_4);
        simulation_0.getReportManager().removeObjects(maxReport_1, minReport_0, sumReport_2);
